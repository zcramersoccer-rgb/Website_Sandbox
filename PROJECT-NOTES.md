...
## UNVERIFIED: the planting price ladder (flagged 2026-09-23)

**Ask Zach to confirm or correct these three figures.** They are published on SEVEN pages and I can
find no source for them anywhere.

| Published as | Where |
|---|---|
| Plant beds **$1,000 - $2,000** | /landscape-installation/, /johns-island-sc/, /kiawah-island-sc/, /daniel-island-sc/, /folly-beach-sc/, /sullivans-island-sc/, /blog/outdoor-living-space-cost-charleston/ |
| Front yard planting **$5,000 - $10,000** | same |
| Full house landscape **$20,000+ in plants** | same |

**What was checked, 2026-09-23.** They appear in none of the three fact files - not this file, not
`site-build/town_page_facts.md`, and not `content-answers.md`, which is the actual record of Zach's
answers and contains no dollar figures at all. They DO appear in `Landscaping-Agent/knowledge.md`,
whose header asserts "Prices: only figures published on the website, which Zach confirmed" - but that
file is generated by `build_knowledge.py` FROM the live site, so it is a copy of the claim, not a
source for it. Circular.

**Left published deliberately.** They almost certainly came from Zach verbally during the original
copy pass, and deleting real prices from seven pages on the strength of a missing note would be the
worse error - price ranges are among the most useful things on those pages. But they are the only
published figures on this site with no traceable source, so they are recorded here rather than left
to look sourced.

**If Zach confirms:** move them into a dated VERIFIED block like the other answers and delete this
section. **If he corrects them:** seven pages plus `town_pages.py`, `page_landscape_installation.py`
and the blog cost post carry them, and `build_knowledge.py` must be re-run afterwards.
